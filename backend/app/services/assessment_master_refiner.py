...
CANDIDATE_SYSTEM = (
    ...
    "\\begin{array}{column-spec}...\\end{array} in the house style "
    "(pipe columns {|c|c|c|}, \\hline above and below every row, a "
    "\\phantom placeholder sized like a plausible real entry — e.g. "
    "\\phantom{7}, never a literal \\phantom{n} — holding empty cells "
    "open, prose in \\text{...}, \\\\[0.12 cm] between a context sentence "
    "and the array — owner-corrected reference, 2026-08-29). Within the "
    "fields you may edit, a table shipped as coordinate-labelled prose "
    "(\"Table row 1, column 2: 8611\") with every cell present is "
    "re-rendered in that canonical style; labelled prose is a transport "
    "encoding, not a shippable rendering. For a multipart "
