...
ANALYSIS_SYSTEM = _SHARED + column_spec.TEACHING_QUALITY + column_spec.CONCEPT_QUALITY + (
    ...
    "'Description:' label and no other section inside it. " + "Write every learner-facing sentence in the voice of a teacher speaking to the learner and state facts directly: never refer to 'the source', 'the text', 'the chapter states', the evidence packet or these instructions, and never include a note meant for the author, reviewer or evaluator (for example 'observations should be recorded rather than assumed in advance'); an activity is taught by saying what the learner does and what is observed. Do not transcribe a figure's caption, its alt text or a 'Source visual'/'Fig. N –' label into the Description; figures are attached by the placement pass with their own captions — teach what the figure shows in your own words, in a complete sentence. " + "For a poem, "
    "story, or other literary source, quote ONLY the exact line or two "
    "the teaching point needs, clearly marked as a quotation — never "
    ...
    "shared or paraphrased between concepts. " + "Write it in the imperative register — the capability itself, verb first, addressed to the learner (e.g. 'Identify the parts of a flower and state the role of each'); never open with 'A learner can', 'The learner can', 'The student…' or 'Students will', and never restate the Description. " + "Misconceptions and Error "
...
PRELEARN_CAPTURE_SYSTEM = _SHARED + (
    ...
    "for what it teaches — this chapter's own NEW teaching is never a "
    "prerequisite for itself. One deliberate exception (owner ruling, "
    "2026-08-27): material the chapter itself presents as revision of "
    "EARLIER learning — an opening basics/recap/warm-up passage that "
    "re-teaches what a previous year established before the new teaching "
    "begins — is prior knowledge stated in the chapter's own words, not "
    "new teaching. Capture what such a passage revises as prerequisites. "
    "Judge that from what the passage actually does with the learner's "
    "existing knowledge, never from a banner word alone: a warm-up that "
    "opens the NEW material is not revision, and a revision passage "
    "without any banner still is. text names the assumed element precisely "
...
PRELEARN_CRITIC_SYSTEM = _SHARED + column_spec.REVIEW_QUALITY + (
    ...
    "chapter — flag anything that restates the chapter's own NEW "
    "teaching as a prerequisite (material the chapter itself presents as "
    "revision of earlier learning is prior knowledge, not new teaching, "
    "and capturing it is correct), anything the cited evidence does not actually "
...
PREMAP_SYSTEM = _SHARED + column_spec.TEACHING_QUALITY + column_spec.CONCEPT_QUALITY + (
    ...
    "space), never an array. " + "Write every topic title and concept_title in Title Case — capitalise each principal word, keep connector words (of, and, the, to) lowercase, and keep acronyms, symbols and units exactly as the source spells them (DNA, pH, NaCl); keep connector words lowercase except as the first word. " + "A pre-learning "
    ...
    "it is held, distinct for every concept. " + "Write it in the imperative register — the capability itself, verb first, addressed to the learner (e.g. 'Identify the parts of a flower and state the role of each'); never open with 'A learner can', 'The learner can', 'The student…' or 'Students will', and never restate the Description. " + "Write every learner-facing sentence in the voice of a teacher speaking to the learner and state facts directly: never refer to 'the source', 'the text', 'the chapter states', the evidence packet or these instructions, and never include a note meant for the author, reviewer or evaluator (for example 'observations should be recorded rather than assumed in advance'); an activity is taught by saying what the learner does and what is observed. Do not transcribe a figure's caption, its alt text or a 'Source visual'/'Fig. N –' label into the Description; figures are attached by the placement pass with their own captions — teach what the figure shows in your own words, in a complete sentence. " + "Carry no 'Description:' or "
...
PREMAP_CRITIC_SYSTEM = _SHARED + column_spec.REVIEW_QUALITY + column_spec.CONCEPT_QUALITY + (
    ...
    "does any pre-learning concept carry this chapter's own NEW content — "
    "its explanations, its examples, or its questions — rather than the "
    "prior knowledge it assumes? (A fundamental the chapter's own opening "
    "basics/recap passage revises from earlier learning is assumed prior "
    "knowledge, not leakage.) Response schema: {\"verdict\": "
    "\"verified|rejected\", \"confidence\", \"issues\": [..]}. You are "
...
PREQUESTIONS_AUTHOR_SYSTEM = _SHARED + (
    ...
    "run instructions. Solve each task before returning it. " + 'Author each question in the response form you intend: a fill-in shows its blank, a selection question lists its options, and a question asking for a reason or explanation asks for it in a sentence; do not leave the response form for a later stage to invent. ' + "question_text "
...
HOST_SYSTEM = _SHARED + (
    ...
    "exercises place purely by what they ask. A question whose "
    "difficulty_hint is Advanced, or whose method needs a concept taught "
    "LATER in topics_in_teaching_order, belongs with that later concept, "
    "never with an earlier concept it is printed beside. Prefer the most "
    "granular method/application/modeling concept; never file a task "
    "under a nearby definition, broad formula, or final concept merely "
    "for convenience. A textbook Activity, experiment or discussion unit "
    "goes to the related NORMAL concept, never to a Culmination; major "
    "concepts assessed by exercises receive their own Types, never parked "
    "only on a Culmination. Surface similarity is "
...
PLACE_CRITIC_SYSTEM = _SHARED + column_spec.REVIEW_QUALITY + (
    ...
    "concept's teaching. Also flag placement of an isolated typographic "
    "operator/glyph crop that adds no visual teaching content; it should be "
    "text plus a decorative_or_duplicate disposition. Response schema: {\"verdict\": "
    "\"verified|rejected\", \"confidence\", \"issues\": [..]}. You are "
    "an auditor, not a judge: your dissent is recorded on the output "
    "for human review and does not block the run."
)
...
FIXER_SYSTEM = _SHARED + (
    ...
    "the SAME response schema the original provider was asked for; infer "
    "that schema from last_response and the rules in original_payload, and "
    "when the request names its own response schema, use that one. Add a "
    "top-level \"rationale\" field: one or two sentences stating what was "
    "blocked, what you decided, and why. The rationale is a PROTOCOL field, "
    "not part of the artifact: where the caller's schema forbids extra keys "
    "the server separates it for you, so a complaint about an unexpected "
    "\"rationale\" is never something to negotiate and never a reason to "
    "withhold your reasoning. Always send it. "
...
def instruction_rules_suffix(
    ...
    *,
    ...
    from .. import source_topic_policy, prelearning_capture_policy, prelearning_foundation_policy, generation_quality_policy, generation_repair_policy

    policy_suffix = ""
    if isinstance(metadata, Mapping):
        if metadata.get("source_topic_policy_version") == source_topic_policy.SOURCE_TOPIC_POLICY_VERSION:
            policy_suffix += " OWNER SOURCE POLICY: " + (
                source_topic_policy.PRE_SOURCE_POLICY
        ...
        if slots not in (PRE_LEARNING_SLOTS, PRE_QUESTION_SLOTS):
            boundary_metadata = {
                ...
                if key not in (prelearning_foundation_policy.KEY, generation_quality_policy.KEY, generation_repair_policy.KEY)
            }
    ...
    if not parts:
        return policy_suffix
    return " RUN INSTRUCTIONS (Architect): " + " ".join(parts) + policy_suffix
