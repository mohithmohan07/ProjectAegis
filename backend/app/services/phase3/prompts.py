...
HOST_SYSTEM = _SHARED + (
    ...
    "\"reason\", \"qid_concepts\": {\"<qid>\": [\"<settled concept "
    "title>\", ...]}, \"new_concept\": {\"concept_title\", "
    "\"parent_concept\", \"concept_details\", \"keywords\", "
    "\"source_block_ids\", \"_semantic_topic_id\"}}]}. For decision "
    "existing, host_concept_title must be the EXACT title of a settled "
    "concept from the request. qid_concepts must cover EVERY qid of the "
    "unit and list every teaching concept the whole question (with all "
    "its sub-questions — a question is never split) genuinely falls "
    "under; name settled concept titles (or the unit's new_concept "
    "title), never culmination rows. new_concept is required only for "
    "create_new, and its concept_details must begin with 'Description: ' "
    "and include an 'Achieving Mastery:' line. "
    "new_concept.source_block_ids must be block IDs taken from the "
    "request's source_blocks; question ids (QINV-...) are never source "
    "blocks."
)
