...
MARKING_SYSTEM = column_spec.OUTPUT_DISCIPLINE + column_spec.ASSESSMENT_QUALITY + (
    ...
    "sum exactly to that subquestion's marks. Those two rules couple: with "
    "every keyword weight a positive multiple of 0.5, a subquestion with K "
    "keywords needs at least 0.5 x K marks, so give such a subquestion "
    "enough of the cell total to pay each keyword rather than splitting "
    "one mark four ways. Do not use negative values, "
...
def _live_author(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return generation._openai_json(
        str(payload.get("rules") or MARKING_SYSTEM),
        suffix,
...
def _live_critic(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return generation._openai_json(
        str(payload.get("critic_rules") or MARKING_CRITIC_SYSTEM),
        suffix,
