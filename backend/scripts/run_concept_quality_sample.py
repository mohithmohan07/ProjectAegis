...
def _mock_stages() -> dict:
    ...
    desc = [
        ...
        _row("Meaning of Exponents", "Parts of a Power", "Identifying Base and Exponent", "Description: A power has two parts: the base, which is the repeated factor, and the exponent, which tells how many times it is used. Students use this idea when reading or writing expressions such as [Katex] 9^5 [/Katex]. Mastery means naming both parts and explaining their roles without confusing them.", "base, exponent, power"),
        _row("Meaning of Exponents", "Exponent Expansion", "Expanding Powers into Repeated Multiplication", "Description: Expanding a power means writing the base as a repeated factor the number of times shown by the exponent. This is used to connect compact exponent notation with multiplication. Mastery means expanding expressions such as [Katex] 4^3 [/Katex] correctly and preserving the base each time.", "expand, powers, factors"),
        _row("Laws of Exponents", "Operations on Powers with the Same Base", "Multiplying Powers with the Same Base", "Description: Multiplying powers with the same base uses the rule [Katex] a^m \\times a^n = a^{m+n} [/Katex]. Students should recognize that the base remains unchanged while the exponents are added because the repeated factors combine into one longer product. Mastery means choosing this rule only when the bases are the same.", "exponents, same base, multiplication, powers, index law"),
        _row("Laws of Exponents", "Operations on Powers with the Same Base", "Dividing Powers with the Same Base", "Description: Dividing powers with the same base uses the rule [Katex] a^m \\div a^n = a^{m-n} [/Katex], where [Katex] a \\ne 0 [/Katex] and [Katex] m > n [/Katex]. The base remains unchanged while common repeated factors cancel. Mastery means applying the rule only when the base is non-zero and the expression fits the stated condition.", "exponents, same base, division, non-zero base"),
        _row("Laws of Exponents", "Powers of Powers", "Simplifying a Power Raised to a Power", "Description: A power raised to another power uses the rule [Katex] (a^m)^n = a^{mn} [/Katex]. This matters when an exponential expression repeats an already formed power. Mastery means multiplying the exponents, keeping the base unchanged, and distinguishing this case from same-base multiplication.", "power of power, exponent multiplication, powers"),
        _row("Negative and Zero Exponents", "Zero Exponent Rule", "Simplifying Zero Exponents", "Description: Any non-zero number raised to the power zero equals 1, so [Katex] a^0 = 1 [/Katex] where [Katex] a \\ne 0 [/Katex]. Students use this rule to simplify powers without expanding them. Mastery means applying the result only for non-zero bases and avoiding the error of making the value zero.", "zero exponent, non-zero base, simplify"),
        _row("Negative and Zero Exponents", "Negative Exponent Rule", "Converting Negative Exponents to Reciprocal Form", "Description: A negative exponent shows reciprocal form: [Katex] a^{-m} = \\frac{1}{a^m} [/Katex] where [Katex] a \\ne 0 [/Katex]. This is used to rewrite powers with positive exponents in fractions. Mastery means moving correctly from negative exponent notation to reciprocal notation without changing the base incorrectly.", "negative exponent, reciprocal, non-zero base"),
