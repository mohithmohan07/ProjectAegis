...
def test_prepare_final_routes_through_the_rewrite(
    ...
    from app.services import analysis_correction_policy, model_provider, prelearning_capture_policy, prelearning_foundation_policy, source_topic_policy
    ...
    # Q68: the paired-correction policy is frozen on new envelopes only.
    expected["metadata"][analysis_correction_policy.KEY] = analysis_correction_policy.VERSION
...
def test_historical_sealed_envelope_keeps_legacy_capture_policy(monkeypatch, tmp_path, fixture_env):
    ...
    from app.services import analysis_correction_policy, prelearning_capture_policy, prelearning_foundation_policy
    ...
    assert analysis_correction_policy.KEY not in original["metadata"]
