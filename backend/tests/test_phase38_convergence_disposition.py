...
def test_no_failure_mode_ends_the_run_without_a_candidate(
    ...
    Whatever comes back from below Phase 3.8 -- a grounding rejection, a
    contract defect, an unbridgeable pause -- the caller receives a complete
    candidate. There is no input to this stage that produces no workbook.
    """
    monkeypatch.setenv("AEGIS_PHASE38_TOPOLOGY_CONVERGENCE_PASSES", "2")
    records = _records()
...
def test_the_disposition_narrows_against_the_real_source(monkeypatch):
    """End to end: the concept that would not ground ships, saying less.

    The other tests run with no active graph, which exercises the "nothing to
    narrow against" branch. This one supplies canonical blocks so the actual
    rewrite runs through Phase 3.8.
    """
    monkeypatch.setenv("AEGIS_PHASE38_TOPOLOGY_CONVERGENCE_PASSES", "1")
    source = (
        "The Frankfurt Parliament was convened in the Church of St Paul in "
        "May 1848. It drafted a constitution for a German nation to be "
        "headed by a monarchy subject to parliament."
    )
    monkeypatch.setattr(
        phase38.phase3,
        "active_graph",
        lambda: {
            ...
            "topics": [{"topic_id": "T-33", "title": "The Age of Revolutions"}],
            "blocks": [{"block_id": "B33", "topic_id": "T-33",
                        "kind": "paragraph", "order": 10}],
        },
    )
    monkeypatch.setattr(
        phase38.phase3,
        "active_session",
        lambda: {"canonical": {
            "blocks": [{"block_id": "B33", "display_text": source}]
        }},
    )

    rows = [{
        "concept_title": "The Frankfurt Parliament",
        "topic": "The Age of Revolutions",
        "_semantic_topic_id": "T-33",
        "parent_concept": "Parent",
        "_phase32_origin_concept_id": "TOPOLOGY-CONCEPT-0001",
        "concept_details": (
            "Description: The Frankfurt Parliament drafted a constitution "
            "for a German nation. Its delegates were later exiled to Zurich "
            "and Basel by the Prussian army. // "
            "Achieving Mastery: The learner explains the assembly's aims."
        ),
    }]

    def always_fails(_records, *_args, **_kwargs):
        raise ValueError(
            "failed exact source-block grounding before freeze for topic "
            "'The Age of Revolutions': CONCEPT-GROUND-0001 is not fully "
            "supported by B33"
        )

    with phase38.convergence_checkpoint_context(
        scope="upload-job:narrowing",
        state=None,
        persist=lambda _expected, _value: None,
    ):
    ...
    assert len(result) == 1
    claim = result[0]["concept_details"]
    # The supported clause survives; the invented one is gone and recorded.
    assert "drafted a constitution" in claim
    assert "Zurich" not in claim
    assert result[0][phase38.evidence_narrowing.NARROWED_KIND_FIELD] == (
        "narrowed")
    assert any(
        "Zurich" in clause
        for clause in result[0][
            phase38.evidence_narrowing.NARROWED_DROPPED_FIELD
        ]
    )
    # Placement and pedagogy are untouched by a grounding disposition.
    assert result[0]["topic"] == "The Age of Revolutions"
    assert "Achieving Mastery: The learner explains" in claim
