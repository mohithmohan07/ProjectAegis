...
| D3 | Unchanged lane without an upload: keep Q49 (re-render the generated workbook and extract it by API; one author + critic per lane) or adopt the staged Step 1 rows mechanically. | **Owner approved 11 September 2026: keep Q49.** One authority path; Step 1 rows never re-enter Step 2. |
| D4 | Skip the Q18 pre-learning claim and the P3 source-duplicate verdict when the Post inventory is a reviewed file. | **Owner approved 11 September 2026. Implemented:** both are skipped for a reviewed Post inventory, with an explicit disposition on the release and a log line, so a skip is never read as a verdict of none. Source-extracted banks and the Pre lane's generated dedup are unchanged. |
| D5 | Trust the reviewed-file author's placement of governing instructions instead of the regex fold on reviewed items. | **Owner approved 11 September 2026. Implemented:** a row carrying reviewed-file provenance keeps the placement its extraction author decided; the pattern is never applied to it and a neighbouring source directive cannot absorb it. Each exemption is recorded and the zero-loss accounting is unchanged. |
...
| D7 | Gate the legacy force-release, revisions and release-review routes off for review-workflow jobs. | **Owner approved 11 September 2026. Implemented:** the five mutating acts refuse with a message naming the step that owns the correction; the read-only history projections stay open; historical jobs keep every route. |
...
| D9 | Step 03 Master edits: every question-band cell applied verbatim; omissions honoured; rows with an unknown or blank label. | Implemented. **Owner approved 11 September 2026:** Step 03 now also accepts a brand-new question written under an existing group — applied verbatim through the same projection an edit uses, minted a durable label, recorded as reviewer-authored on the row's own audit with no invented source provenance. A row naming no resolvable group is still refused readably. |
...
| D14 | A second reviewed Master round for a lane already published updates the database rows; the shared CMS output workbook was append-only, so that row kept the earlier wording. | **Owner approved 11 September 2026. Implemented:** the writer takes an explicit set of labels a caller may rewrite and re-projects only those rows' question band in place, under the same lock and outbox; a caller naming nothing keeps the append-only behaviour byte for byte. The publication names the labels its upload reported as updated. |
...
## 9. Owner approvals of 11 September 2026

Everything §7 held open was decided in one round. D3 keeps Q49 unchanged; D4,
D5, D7, D9's addition path and D14 are implemented above and recorded in
register entry Q52. D6 (the Q31 critic stack and the per-row Concept Refiner),
D8 (mechanical display rendering of file-supplied Pre questions) and D13 (the
orphaned frontend surfaces) stand as they were.
