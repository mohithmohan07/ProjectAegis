...
| ID | Checkable property | Polarity | Source and reason |
|---|---|---|---|
| QID-01 | Classification precedes embedding and allotment; every final Example has a stable QID route, Case identity where applicable, and one owner. | Named duplicate/unknown B1 identities BLOCK; `example_qid_missing` is OPEN; semantic classification is ADVISORY | `docs/aegis-restructure.md:364-385`. A Type may legitimately render under multiple concepts when different Cases have different owners. |
